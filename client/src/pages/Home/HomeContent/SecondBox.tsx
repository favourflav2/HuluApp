...
import { openMovieItemDetials } from "../../../redux/features/huluSlice";

function SecondBox() {
  const { data, isFetching, error } = useGetMovieByIdQuery(68718);
  ...
  return isFetching ? (
    <Skeleton variant="rectangular" className="md:max-h-[700px] h-[650px] w-full bg-gray-400 mb-5 mt-5" />
  ) : (
    <div className="w-full h-auto flex flex-col mb-5 mt-5">
      {/* Content */}
      <div className="w-full h-auto flex">
        <div className=" relative w-full h-auto">
          <img src={`https://image.tmdb.org/t/p/original/${data?.backdrop_path}`} alt="" className="imgShadow brightness-50 hue-rotate-15 object-cover w-[100%]  lg:h-[650px] xl:h-[700px] h-[550px]" />

          {/* Desktop Content */}
          <div className="w-auto px-5 py-2 h-auto  absolute  md:bottom-[120px] md:left-10 md:flex hidden  flex-col firstBox">
            {/* Title */}
            <h1 className="text-white mb-3">BASED ON MY FAVORITE MOVIE</h1>

            {/* Show Title */}
            <div className="w-auto flex flex-col h-auto text-white mb-2">
              <h1 className="flex text-sm">Blu-Ray</h1>
              <h1 className="text-[20px] font-semibold flex ">{data?.title}</h1>
            </div>
            ...
            <div className="w-full flex items-center mb-2">
              ...
              {/* Details */}
              <button
                className="px-6 py-4 bg-inherit border border-white text-white rounded-lg mr-2 transition ease-in-out delay-150 duration-250 hover:border-gray-500 hover:text-gray-500"
                onClick={() => {
          ...
          <div className="min-w-[390px] px-5 py-2 h-auto  absolute  mobileFirstBox  md:hidden flex flex-col mobileFirstBox2 items-center justify-center">
            ...
            <div className="w-full flex justify-center items-center mb-2">
              ...
              {/* Details */}
              <button
                className="px-6 py-4 bg-inherit border border-white text-white rounded-lg mr-1 transition ease-in-out delay-150 duration-250 hover:border-gray-500 hover:text-gray-500"
                onClick={() => {
                  dispatch(openMovieItemDetials(pathname));
                  navigate(`/movieDetails/${data?.id}`);
                }}
